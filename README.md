...
- [GHIF Practical guidelines for quality control of WGS results in population-scale initiatives](#ghif-practical-guidelines-for-quality-control-of-wgs-results-in-population-scale-initiatives)
- [Discussion points for GHIF meeting 26/10/21](#discussion-points-for-ghif-meeting-261021)
- [Documents](#documents)
...
* Some of the required "field" metrics such as base quality are difficult to filter or change at the BAM stage (without creating a entirely new BAM file). These include:
...
These may affect some of the calculated QC metrics if the relevant information is not provided as metadata along with the BAM files. How to assess QC metrics if they are unknown? That is if unmapped reads are not included in BAM file, is "Contamination" QC metric assessable?

* Common programs for dealing with BAM files seem to allow filtering bases on mapping quality but not base quality
    * That is BQ filter available for samtools coverage but not mosdepth nor samtools stats
...
### Alternative method using mosdepth
